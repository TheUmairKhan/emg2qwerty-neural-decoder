...
class WindowedEMGDataset(torch.utils.data.Dataset):
    ...
    def __post_init__(
        ...
        with EMGSessionData(self.hdf5_path) as session:
            ...
            prompt = LabelData.from_prompts(session.prompts)
    ...
    def collate(samples: Sequence[dict[str, Any]]) -> dict[str, Any]:
        """Collates a list of samples into a padded batch of inputs, targets and prompts.
        ...
        """
        inputs = [sample["emg"] for sample in samples]  # [(T, ...)]
        targets = [sample["labels"] for sample in samples]  # [(T,)]
        prompt = samples[0]["prompts"]
        session_name = samples[0]["session_id"]
